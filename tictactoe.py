...
class TicTacToeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("TicTacToe hijriah")
    ...
    def create_buttons(self):
        for i in range(3):
            for j in range(3):
                button = tk.Button(self.root, text='', font=('arial', 40), width=5, height=2, bg='black',fg='white', command=lambda row=i, col=j: self.on_button_click(row, col))
    ...
    def end_game(self):
        ...
        if winner_val:
            messagebox.showinfo("GAME SELESAI", f"PEMENANGNYA : {winner_val}")
        else:
            messagebox.showinfo("GAME SELESAI", "GAME SERI")
        self.reset_board()
